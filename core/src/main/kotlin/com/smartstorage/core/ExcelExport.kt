...
object ExcelExport {
    fun write(s: Inventory, output: OutputStream, filter: Filter? = null) {
        ...
        val sheets: List<Pair<String, List<List<Any?>>>> = listOf(
            "物品汇总" to (listOf(listOf("物品 ID", "名称", "分类", "标签", "单位", "当前数量", "已知成本总额（元）", "缺价数量", "贵重物品", "位置")) +
                s.items.filter { it.id in ids }.map { item -> val matches = rows.filter { it.item.id == item.id }
                    listOf(item.id, item.name, item.category, item.tags.mapNotNull { labels[it] }.joinToString("、"), item.unit,
            ...
            "批次与位置" to (listOf(listOf("批次 ID", "物品 ID", "名称", "位置", "数量", "单位", "单价（元）", "购买日期", "到期日", "状态")) + rows.map {
                listOf(it.batch.id, it.item.id, it.item.name, s.path(it.balance.placeId), it.balance.quantity, it.item.unit,
